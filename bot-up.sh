...
# Player.log resets on every game relaunch, so after triggering a recovery
# we re-grep the log from scratch (old lines about the flake are gone).
wait_for_signal() {
  local elapsed=0
  while (( elapsed < TIMEOUT_S )); do
    ...
    if grep -q "SteamAPI.Init() returned False." <<<"$log_contents"; then
      ...
      if (( AUTO_RECOVER == 1 && recover_once_done == 0 )); then
        ...
        recover_once_done=1
        # Player.log resets on relaunch — subsequent iterations re-grep fresh content.
      else
        return 1
      fi
    fi
